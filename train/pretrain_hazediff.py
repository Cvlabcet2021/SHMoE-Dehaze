...
from models.hazediff_cpg import HazeDiffCPG
...
def main():
    ...
    dcfg = cfg["hazediff_cpg"]
    model = HazeDiffCPG(
        image_channels=3,
        base_channels=dcfg["base_channels"],
        num_res_blocks=dcfg["num_res_blocks"],
        diffusion_timesteps=dcfg["diffusion_timesteps"],
        sampling_steps=dcfg["sampling_steps"],
        guidance_scale=dcfg["guidance_scale"],
    ).to(device)
    ...
    for epoch in range(epochs):
        ...
        for batch in loader:
            ...
            out = model(hazy, target=gt, sample=False)
            loss = torch.mean(
                (out["noise_prediction"] - out["noise_target"]) ** 2
            )
    ...
    torch.save(
        model.state_dict(),
        "checkpoints/hazediff_pretrained.pth",
    )
